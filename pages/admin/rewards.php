...
$members = rows(<<<'SQL'
    ...
    ORDER BY CASE u.ambassador_tier WHEN 'lead' THEN 1 WHEN 'senior' THEN 2 ELSE 3 END, u.name
SQL);
...
<div class="reward-overview">
    <div><p class="eyebrow">MÔ HÌNH GHI NHẬN</p><h2>Thưởng theo kết quả, chất lượng và cấp độ</h2><p>Điểm được tính từ lead hợp lệ, nội dung UGC, chất lượng tư vấn và hệ số phân hạng.</p></div>
    <div class="reward-formula"><span>Lead × CPL</span><i class="bi bi-plus-lg"></i><span>UGC + bonus</span><i class="bi bi-plus-lg"></i><span>Chat quality</span><i class="bi bi-x-lg"></i><strong>Tier</strong></div>
</div>
...
<div class="row g-4 metric-grid mb-4">
    ...
    <div class="col-md-4"><article class="metric-card"><span class="metric-icon violet"><i class="bi bi-trophy-fill"></i></span><div><p>Lead toàn đội</p><h3><?= (int) scalar('SELECT COALESCE(SUM(leads),0) FROM affiliate_links') ?></h3><small>Đã ghi nhận từ affiliate</small></div></article></div>
</div>

<section class="panel-card">
    <div class="panel-head"><div><p class="eyebrow">HỒ SƠ AMBASSADOR</p><h3>Điều kiện, phân hạng và vi phạm</h3></div><span class="panel-chip">Junior 1.0× | Senior 1.3× | Lead 1.5×</span></div>
    <div class="table-responsive"><table class="table clean-table reward-table align-middle"><thead><tr><th>Thành viên</th><th>Điều kiện</th><th>Hiệu quả</th><th>Cấp độ</th><th>Chính sách</th><th>Vi phạm</th><th></th></tr></thead><tbody>
        ...
        <tr>
            ...
            <td><strong><?= (int) $member['leads'] ?> leads</strong><small class="d-block text-muted">Chat <?= (int) $member['chat_quality'] ?>/100 | <?= number_format((int) $member['points']) ?> điểm</small></td>

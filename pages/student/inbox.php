...
?>
<details class="panel-card inbox-policy mb-3">
    <summary><span><i class="bi bi-shield-check"></i> Quyền riêng tư và kiểm duyệt</span><i class="bi bi-chevron-down"></i></summary>
    <p>Tin nhắn được kiểm tra tự động. Người phụ trách chỉ xem nội dung bị đánh dấu và phần hội thoại cần thiết để xử lý. Khi đại sứ chọn Ask AI, lịch sử hội thoại được lọc dữ liệu nhận dạng trước khi gửi đến mô hình đang cấu hình; AI chỉ gợi ý hướng phản hồi. Câu hỏi chuyển đến Ban Tuyển sinh được gửi riêng, không kèm toàn bộ lịch sử.</p>
</details>
...
<?php if ($selected): ?>
<details class="panel-card p-3 mb-3" data-ai-assist="conversation_ai_assist" data-conversation-id="<?= $selected ?>">
    <summary>AI hỗ trợ hội thoại</summary>
    <p class="helper-note mt-3">Khi tạo gợi ý, hệ thống gửi tối đa 20 tin nhắn gần nhất đến mô hình AI đang cấu hình. Một số dữ liệu nhận dạng được lọc tự động. Không sử dụng tính năng này với thông tin nhạy cảm và luôn kiểm tra nội dung trước khi gửi.</p>
    <button class="btn btn-sm btn-outline-brand" type="button" data-ai-run>Tạo tóm tắt và gợi ý trả lời</button>
...
<div class="inbox-shell" data-inbox-conversation="<?= $selected ?>">
    ...
    <section class="inbox-room"><?php if ($selected): $info = rows('SELECT c.*, p.name AS prospect_name, p.major FROM conversations c JOIN users p ON p.id=c.prospect_id WHERE c.id=? AND c.ambassador_id=?', [$selected,user()['id']])[0] ?? null; ?><div class="inbox-room-head"><span class="avatar avatar-sm"><?= e(initials($info['prospect_name'])) ?></span><div><strong><?= e($info['prospect_name']) ?></strong><small>Quan tâm: <?= e($info['major'] ?: 'Chưa xác định') ?></small></div><span class="quality-chip"><i class="bi bi-activity"></i> Điểm chất lượng <?= (int) $info['quality_score'] ?>/100</span><span class="safety-badge"><i class="bi bi-shield-check"></i> Kiểm duyệt tự động</span></div><div class="message-list inbox-messages" id="messageList"></div><form class="chat-composer inbox-composer" id="messageForm"><input type="hidden" id="conversationId" value="<?= $selected ?>"><textarea id="messageInput" rows="1" placeholder="Nhập tin nhắn..."></textarea><button type="submit" aria-label="Gửi tin nhắn"><i class="bi bi-send-fill"></i></button></form><?php else: ?><div class="empty-state"><i class="bi bi-chat-heart"></i><h2>Chưa có tin nhắn</h2><p>Khi học sinh chọn trò chuyện cùng bạn, cuộc hội thoại sẽ xuất hiện tại đây.</p></div><?php endif; ?></section>
        ...
        <div class="modal-content">
            <form method="post" action="actions.php?action=escalate_question">
                ...
                <div class="modal-header">
                    <div>
                        <p class="eyebrow mb-0 text-warning">CÂU HỎI CẦN XÁC NHẬN</p>
                ...
                <div class="modal-body">
                    ...
                    <label class="form-label fw-semibold" for="escalateReason">Nội dung cần xác nhận</label>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-brand" type="submit"><i class="bi bi-send-check"></i> Gửi Ban Tuyển sinh</button>
                </div>
            </form>
        </div>
    </div>

            ...
            <?php foreach ($ambassadors as $index => $ambassador): ?>
                <div class="col-md-6 col-lg-4">
                    <article class="ambassador-card h-100">
                        <div class="ambassador-cover cover-<?= ($index % 3) + 1 ?>"><span class="profile-monogram"><?= AmbassadorProfiles::avatarHtml($ambassador) ?></span><?php if ($ambassador['is_online']): ?><span class="online-badge"><i></i> Đang online</span><?php endif; ?></div>
                        <div class="ambassador-body"><span class="verified"><i class="bi bi-patch-check-fill"></i> Đại sứ sinh viên</span><h3><?= e($ambassador['name']) ?></h3><p class="major"><i class="bi bi-book"></i> <?= e($ambassador['major']) ?> | Năm <?= (int) $ambassador['study_year'] ?></p><p><?= e($ambassador['bio']) ?></p><div class="tag-row"><?php foreach (array_slice(explode(',', $ambassador['interests']), 0, 2) as $interest): ?><span><?= e(trim($interest)) ?></span><?php endforeach; ?></div><a class="btn btn-card w-100 mt-3" href="index.php?page=ambassadors&ambassador=<?= (int) $ambassador['id'] ?>">Mở cuộc trò chuyện <i class="bi bi-arrow-up-right"></i></a></div>
                    </article>
                </div>
            <?php endforeach; ?>
        </div>
